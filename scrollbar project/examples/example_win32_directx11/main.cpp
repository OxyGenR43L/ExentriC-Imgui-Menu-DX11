...
namespace image
{
    ...
    ID3D11ShaderResourceView* folder = nullptr;
...
namespace addition
{
    const char* cheat_name = "PAST OWL";
    const char* user_name = "OWL";
...
int main(int, char**)
{
    ...
    if (image::folder == nullptr) D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, folder, sizeof(folder), &info, pump, &image::folder, 0);
        ...
        ImGui::NewFrame();
        {
            ...
            if (addition::menu_opticaly > 0.01f) {
                ImGui::Begin("NULL", nullptr, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBringToFrontOnFocus);
                {
                    ...
                    ImGui::BeginGroup();
                    {
                        const char* tabs_array[] = { "Aimbot", "Visuals", "Skins", "Folder", "Settings" };
                        ID3D11ShaderResourceView* image_array[5] = { image::aimbot, image::visuals, image::skins, image::folder, image::settings };
                        ...
                        if (active_tab == 0) {
                            ...
                            ImGui::BeginGroup();
                            {
                                ImGui::CustomBeginChild("AIMBOT", ImVec2((region.x / 2) - spacing.x - (spacing.x / 2) - (170 / 2), (region.y - 48 - (spacing.y * 3)) / 2));
                                {
                                    ...
                                    if (ImGui::Button("Succesfully", ImVec2((ImGui::GetContentRegionMax().x - style->WindowPadding.x - spacing.x / 2) / 2, 25))) ImGui::Notification({ ImGuiToastType_Success, 2000, "I'm here because it's successful!" });
                                    ImGui::SameLine();
                                    if (ImGui::Button("Warning", ImVec2((ImGui::GetContentRegionMax().x - style->WindowPadding.x - spacing.x / 2) / 2, 25))) ImGui::Notification({ ImGuiToastType_Warning, 2000, "I'm sad because I was warned :(" });

                                    if (ImGui::Button("Error", ImVec2((ImGui::GetContentRegionMax().x - style->WindowPadding.x - spacing.x / 2) / 2, 25))) ImGui::Notification({ ImGuiToastType_Error, 2000, "It seems to be a failure.." });
                                    ImGui::SameLine();
                                    if (ImGui::Button("Info", ImVec2((ImGui::GetContentRegionMax().x - style->WindowPadding.x - spacing.x / 2) / 2, 25))) ImGui::Notification({ ImGuiToastType_Info, 2000, "Wow! Have I been notified, is there something new?" });

                                    if (ImGui::Button("Config", ImVec2(ImGui::GetContentRegionMax().x - style->WindowPadding.x, 25))) ImGui::Notification({ ImGuiToastType_Config, 2000, "Config saven successfully!!" });
