...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(6, 0), grab_rect.Max - ImVec2(6, 0), GetColorU32(c::accent_color), c::scrollbar::rounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    if (val < 8)
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 26) + 5 : 0.f, g.IO.DeltaTime * 12.f);
    else
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? 160 : 0.f, g.IO.DeltaTime * 12.f);
    ...
    ImGui::SetNextWindowPos(ImVec2(bb.Min.x + 2, bb.Max.y + 5));
